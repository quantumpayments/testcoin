...
function handler(req, res) {
  ...
  wc.getBalance(faucetURI, sequelize, config, function(err, ret){
    if (err) {
      ...
      res.write('<br>\n');
      res.write('Access a paid <a href="/random">resource</a> (cost=25 credits, will be returned to the faucet)')
      res.write('<br>\n');
      res.write('Visit the <a href="/faucet">faucet</a> if you dont have enough credits')
      res.write('<br>\n');
      res.write('Or <a href="/deposit">deposit</a> coins')
      res.write('<br>\n');
      res.write('Or <a href="/withdrawal">withdrawal</a> coins')
      res.write('<br>\n');
      res.end()
    }
    sequelize.close();
